defmodule NodeMonitor do
  alias Phoenix.PubSub
  use GenServer
  require Logger

  @module __MODULE__
  @backoff 1_500

  def start_link(opts) do
    GenServer.start_link(__MODULE__, opts, hibernate_after: 1_000, name: __MODULE__)
  end

  @impl true
  def init(peer) do
    :persistent_term.put(:miner, peer)
    :ok = :net_kernel.monitor_nodes(true)

    {:ok, %{peer: peer, inbox: %{}, conn: false}, {:continue, :init}}
  end

  @impl true
  def handle_continue(:init, %{peer: peer} = state) do
    send(self(), {:connect, peer})

    {:noreply, state}
  end

  @impl true
  def handle_info({:nodedown, node_name}, %{peer: peer} = state) do
    Logger.debug(inspect({:nodedown, node_name}))

    if node_name == peer do
      :timer.send_after(@backoff, {:connect, node_name})

      {:noreply, %{state | conn: false}}
    else
      {:noreply, state}
    end
  end

  def handle_info({:nodeup, node_name}, state) do
    Logger.debug(inspect({:nodeup, node_name}))
    {:noreply, %{state | conn: true}}
  end

  def handle_info({:connect, node_name}, %{peer: peer} = state) do
    if node_name == peer do
      case Node.connect(node_name) do
        false ->
          :timer.send_after(@backoff, {:connect, node_name})

        true ->
          on_connect_spawn(self(), node_name, state.inbox)
      end
    end

    {:noreply, state}
  end

  @impl true
  def handle_cast(
        {:push, pubsub, topic, message},
        %{conn: conn, inbox: inbox, peer: peer} = state
      ) do
    if conn do
      PubSub.direct_broadcast(peer, pubsub, topic, message)
      {:noreply, state}
    else
      {:noreply,
       %{
         state
         | inbox: Map.put(inbox, :rand.uniform(1_000_000_000), {pubsub, topic, message})
       }}
    end
  end

  def handle_cast(:clear, state) do
    {:noreply, %{state | inbox: %{}}}
  end

  @impl true
  def terminate(_reason, _state) do
    :net_kernel.monitor_nodes(false)
  end

  def push(pubsub, topic, message) do
    GenServer.cast(@module, {:push, pubsub, topic, message})
  end

  defp on_connect_spawn(_pid, _node_name, %{}), do: :ok

  defp on_connect_spawn(pid, peer, inbox) do
    spawn_link(fn ->
      for {_key, {pubsub, topic, message}} <- inbox do
        PubSub.direct_broadcast(peer, pubsub, topic, message)
      end

      GenServer.cast(pid, :clear)
    end)
  end
end
