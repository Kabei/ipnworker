PREPARE insert_event(bigint, bytea, integer, bytea, bigint, integer, text)
AS INSERT INTO history.events VALUES($1,$2,$3,$4,$5,$6,$7);

PREPARE last_events(integer, integer)
AS SELECT block_id, hash, "type", "from", size FROM history.events ORDER BY block_id DESC LIMIT $1 OFFSET $2;

PREPARE get_details_event(bytea, bigint)
AS SELECT "args" FROM history.events WHERE hash = $1 AND block_id = $2 LIMIT 1;
