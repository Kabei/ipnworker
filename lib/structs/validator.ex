defmodule Ippan.Validator do
  ...
  defmacro insert(args) do
    quote location: :keep do
      SqliteStore.step("insert_validator", unquote(args))
    end
  end

  defmacro get(id) do
    quote location: :keep do
      SqliteStore.get(:validator, "get_validator", unquote(id), Ippan.Validator)
    end
  end

  defmacro next_id do
    quote location: :keep do
      SqliteStore.one("next_id_validator")
    end
  end

  defmacro exists?(id) do
    quote location: :keep do
      SqliteStore.exists?("exists_validator", [unquote(id)])
    end
  end

  defmacro exists_host?(hostname) do
    quote location: :keep do
      SqliteStore.exists?("exists_host_validator", [unquote(hostname)])
    end
  end

  defmacro owner?(id, owner) do
    quote bind_quoted: [id: id, owner: owner], location: :keep do
      SqliteStore.exists?("owner_validator", [id, owner])
    end
  end

  defmacro total do
    quote location: :keep do
      SqliteStore.one("total_validators", 0)
    end
  end

  defmacro delete(id) do
    quote bind_quoted: [id: id], location: :keep do
      SqliteStore.step("delete_validator", [id])
    end
  end

  defmacro update(map, id) do
    quote location: :keep do
      SqliteStore.update("blockchain.validator", unquote(map), id: unquote(id))
    end
  end
end
